...
int main(int argc, char** argv)
{
	...
	tf::StampedTransform t_w_transform;
	tf::Transform g_t_transform;
	tf::Quaternion g_t_rotation;
	tf::Vector3 t_w_wxyz;
	tf::Vector3 t_t_wxyz;
	double kp = 0.8, ko = 0.5;//TODO
	bool s = false;
	while(!s && ros::ok())
	{
		try
		{// ur_base_link --> base_link
			tf_listener.lookupTransform("base_link", "tool0", ros::Time(0), t_w_transform);
			tf_listener.lookupTransform("base_link", "ar_grasp_3", ros::Time(0), g_w_transform);
			...
			double angular_v = shortest_angle * ko;
			if(angular_v > 0.2)angular_v = 0.2;
			t_t_wxyz = g_t_rotation.getAxis() * angular_v;
	...
	while(ros::ok())
	{
		if(joint_data_come)
		{
			...
			double len = sqrt(tcp_velocities_world(0,0)*tcp_velocities_world(0,0)+tcp_velocities_world(1,0)*tcp_velocities_world(1,0)+tcp_velocities_world(2,0)*tcp_velocities_world(2,0));
			if(len > 0.3)
			{
				tcp_velocities_world(0,0) = tcp_velocities_world(0,0) * 0.3 / len;
				tcp_velocities_world(1,0) = tcp_velocities_world(1,0) * 0.3 / len;
				tcp_velocities_world(2,0) = tcp_velocities_world(2,0) * 0.3 / len;
			}
